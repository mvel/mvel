 ...
 */
public final class StaticMvelParser {
     ...
     * Creating an instance will reduce setup time between parsing files.
     */
    public StaticMvelParser(ParserConfiguration configuration) {
        this(configuration, true /* default to optional semicolon */);
    }

    public StaticMvelParser(ParserConfiguration configuration, boolean optionalSemicolon) {
    ...
    public static ParserConfiguration getStaticConfiguration() {
        return StaticMvelParser.staticConfiguration;
    }

    public static void setStaticTypeSolver(TypeSolver typeSolver) {
        StaticMvelParser.typeSolver = typeSolver;
    }

    public static TypeSolver getStaticTypeSolver() {
        return StaticMvelParser.typeSolver;
    }
    ...
    public static void setStaticConfiguration(ParserConfiguration staticConfiguration) {
        StaticMvelParser.staticConfiguration = staticConfiguration;
    }
    ...
    public <N extends Node> ParseResult<N> parse(ParseStart<N> start, final Provider provider) {
        ...
        for (Supplier<Processor> processor : configuration.getProcessors()) {
            tempProvider = processor.get().preProcess(tempProvider);
        }
        ...
        try {
            ...
            configuration.getProcessors().forEach(postProcessor ->
                    postProcessor.get().preProcess(finalProvider));
